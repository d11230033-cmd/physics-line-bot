...
def handle_message(event):
    ...
    try:
        ...
        (請你「嚴格遵守」 System Prompt 中的「第十一紀元：指令中心」核心邏輯！
        1.  「首先」檢查「原始輸入」是否為「指令 (cmd_...)」。如果是，請「立刻執行指令」。
        2.  「如果不是」指令，才「接著」使用「相關段落」和「評估者邏輯」來回應。)
        """
        prompt_parts = [rag_prompt]

        # --- ★ 專家一：「對話宗師」啟動 (第十一紀元：評估版) ★ ---
